...
function StatusBadge({ status }: { status: SubscriptionStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${colors.bg} ${colors.text}`}
    >
...
function SubscriptionRow({
  ...
  return (
    ...
    >
      ...
      >
        {/* Store icon */}
        <div className="w-10 h-10 shrink-0 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white shadow-md">
        ...
        {/* Plan */}
        <div className="flex-[0_0_130px] min-w-0">
        ...
        >
          ...
          <button
            ...
            className="p-1.5 rounded-lg text-slate-400 hover:text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-500/10 transition"
          >
      ...
      {expanded && (
        <div
          className="border-t border-slate-100 dark:border-white/[0.06] px-5 py-4 bg-slate-50 dark:bg-black/[0.1] grid grid-cols-2 md:grid-cols-4 gap-4 text-xs"
        ...
        >
          {[
            ...
            { label: "Canceled At", value: formatDate(sub.canceled_at) },
            {
              ...
              value: sub.cancels_at_period_end ? "Yes" : "No",
            },
...
export default function StoreSubscriptionsPage() {
  ...
  const handleCreate = async (input: CreateStoreSubscriptionInput) => {
    ...
    if (res.success && res.data) {
      const sub = res.data as { id: string; store_id: string; user_id: string; plan_id: string; billing_cycle: BillingCycle; started_at: string; current_period_start: string; current_period_end?: string | null; expires_at?: string | null };
      ...
      if (plan) {
        ...
        await createInvoice({
          ...
          billing_cycle: input.billing_cycle ?? BillingCycle.MONTHLY,
  ...
  const filtered = subs.filter((s) => {
    ...
    const storeName = s.stores?.store_name?.toLowerCase() ?? "";
    const planName = s.subscription_plans?.name?.toLowerCase() ?? "";
    const ownerEmail = s.stores?.owner?.email?.toLowerCase() ?? "";
    const matchesSearch =
      !q ||
  ...
  // Stats
  const statusCounts = Object.values(SubscriptionStatus).reduce(
    (acc, s) => ({ ...acc, [s]: subs.filter((sub) => sub.status === s).length }),
    ...
    <>
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        <div className="max-w-[1080px] mx-auto px-6 py-10">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Manage subscription plans assigned to stores
              </p>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => router.push("/dashboard/subscription/invoices")}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-100 dark:bg-white/[0.06] hover:bg-slate-200 dark:hover:bg-white/[0.10] text-slate-700 dark:text-slate-300 text-sm font-semibold transition"
              >
          ...
          <div className="grid grid-cols-4 gap-3 mb-6">
            {[
              { label: "Total", value: subs.length, color: "text-slate-700 dark:text-slate-200" },
              { label: "Active", value: statusCounts[SubscriptionStatus.ACTIVE] ?? 0, color: "text-emerald-600 dark:text-emerald-400" },
              { label: "Trialing", value: statusCounts[SubscriptionStatus.TRIALING] ?? 0, color: "text-blue-600 dark:text-blue-400" },
              { label: "Canceled", value: statusCounts[SubscriptionStatus.CANCELED] ?? 0, color: "text-red-600 dark:text-red-400" },
            ].map((s) => (
              <div
                key={s.label}
                className="bg-white dark:bg-white/[0.025] border border-slate-200 dark:border-white/[0.07] rounded-2xl px-4 py-3"
              >
          ...
          <div className="flex gap-3 mb-5">
            <input
              ...
              placeholder="Search by store, plan or owner email…"
              value={search}
          ...
          {!loading && filtered.length > 0 && (
            <div className="flex items-center gap-4 px-5 py-2 text-[10px] font-semibold text-slate-400 dark:text-slate-600 uppercase tracking-widest mb-1">
              ...
              <div className="flex-[0_0_130px]">Plan</div>
              <div className="flex-[0_0_110px]">Status</div>
              <div className="flex-1">Expires</div>
          ...
          {loading ? (
            <div className="flex flex-col gap-3">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="h-16 rounded-2xl bg-slate-200 dark:bg-white/[0.04] border border-slate-200 dark:border-white/[0.06]"
            ...
            <div className="text-center py-20 bg-white dark:bg-white/[0.02] rounded-3xl border border-dashed border-slate-200 dark:border-white/[0.08]">
              ...
              <div className="text-lg font-bold text-slate-500 dark:text-slate-400">
                {search || statusFilter !== "all"
                  ...
                  : "No store subscriptions yet"}
              </div>
              <div className="text-sm text-slate-400 dark:text-slate-500 mt-1">
                {!search && statusFilter === "all" && 'Click "Assign Plan" to create one'}
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-2.5" style={{ animation: "slideDown 0.2s ease" }}>
              {filtered.map((sub) => (
                <SubscriptionRow
                  key={sub.id}
                  sub={sub}
      ...
      >
        ...
        <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 cursor-pointer">
          ...
          />
          Cancel at end of current billing period (instead of immediately)
        </label>
      </Modal>
